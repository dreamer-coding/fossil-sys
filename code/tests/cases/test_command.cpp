...
FOSSIL_TEST_CASE(cpp_test_command) {
    ...
    // Test valid command
    result = fossil_sys_command((char *)"echo Hello World");
    ASSUME_ITS_EQUAL_I32(0, result);

    // Test invalid command
    result = fossil_sys_command((char *)"invalid_command");
...
FOSSIL_TEST_CASE(cpp_test_command_success) {
    ...
    // Test valid command
    result = fossil_sys_command_success((char *)"echo Hello World");
    ASSUME_ITS_EQUAL_I32(0, result);

    // Test invalid command
    result = fossil_sys_command_success((char *)"invalid_command");
    ASSUME_NOT_EQUAL_I32(0, result);
}
...
FOSSIL_TEST_CASE(cpp_test_command_output) {
    ...
    // Test valid command
    result = fossil_sys_command_output((char *)"echo Hello World", output, sizeof(output));
    ASSUME_ITS_EQUAL_I32(0, result);
...
FOSSIL_TEST_CASE(cpp_test_command_exists) {
    ...
    result = fossil_sys_command_exists((char *)"where echo");
#else
    // Use "which" on Unix-like systems
    result = fossil_sys_command_exists((char *)"which echo");
#endif
...
#ifdef _WIN32
    result = fossil_sys_command_exists((char *)"where invalid_command");
#else
    result = fossil_sys_command_exists((char *)"which invalid_command");
#endif
    ASSUME_ITS_EQUAL_I32(0, result);
}
...
FOSSIL_TEST_CASE(cpp_test_command_strcat_safe) {
    ...
    fossil_sys_command_strcat_safe(buffer, (char *)" World", sizeof(buffer));
    ASSUME_ITS_EQUAL_CSTR("Hello World", buffer);

    // Test appending with truncation
    fossil_sys_command_strcat_safe(buffer, (char *)"12345678901234567890", sizeof(buffer));
...
FOSSIL_TEST_CASE(cpp_test_command_erase_exists) {
    ...
    result = fossil_sys_command_erase_exists((char *)"C:\\");
#else
    // Use "/tmp" on Unix-like systems
    result = fossil_sys_command_erase_exists((char *)"/tmp");
#endif

    ASSUME_ITS_EQUAL_I32(1, result);

    // Test non-existing directory
    result = fossil_sys_command_erase_exists((char *)"non_existing_directory");
    ASSUME_ITS_EQUAL_I32(0, result);
}
